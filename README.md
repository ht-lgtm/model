...
내용:학습에 사용할 영상 데이터에서 프레임 단위로 이미지를 추출합니다.

추출된 이미지에 대해 Labeling 작업을 진행하여, 탐지할 객체(승용차, Suv)의 위치를 지정하는 Bounding Box와 클래스 정보를 담은 라벨 파일을 생성합니다.
--> (차종마다 부과되는 벌금이 다르기 때문에)
...
내용:경량화된 객체 탐지 모델인 YOLOv11n을 기반으로 선택하여 빠른 추론 속도와 준수한 성능을 확보하고자 했습니다.
...
-->(원래는 어린이 보호구역 기준 5분이상 주차하면 부과되지만, 시간관계상 영상길이 2분 기준, 30초 이상 주차하면 불법 주정차로 인식 하도록 설정함) 
...
## 느낀점
내가 직접 학습시킨 모델을 사용해 불법 주정차를 추출하고 싶었지만, 예상으론 데이터셋 이슈(해상도, 화각 등) 때문에 모델 학습과정에 문제가 생겼다고 생각함 --> 다른 데이터셋을 학습시켜 다시 테스트해보겠습니다. \

좀 더 구체적으로 어린이 보호구역과 횡단보도 주변을 특정 구역으로 인식시키고 그 구역 안에 있는 차량만 인식하는 기능을 추가해보겠습니다.\

AI를 활용해 만드는 거라 굉장히 쉬울줄 알았지만, 생각보다 힘들었다. AI가 다 해주는게 아니라, 내가 어떻게 잘 프롬프트를 집어넣느냐가 중요하다는것을 느꼈습니다.
